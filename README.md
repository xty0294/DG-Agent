...
- 🎛️ **5 个工具** — AI 自动调用：启动/停止、强度微调、波形切换、短时突增
- 🌊 **可管理的波形库** — 6 个内置波形（呼吸、潮汐、低/中/高脉冲、敲击），支持手动导入 `.pulse` 或 `.zip` 并在设置里编辑名称和说明
- 🛡️ **多层安全护栏** — 用户上限硬夹紧、单回合调用频次限制、AI 幻觉检测自动纠正
...
| 工具 | 说明 | 示例 |
|------|------|------|
| ▶️ `start` | 启动通道：同时设置强度和波形（仅在停止状态下使用） | `start("A", strength=10, waveform="breath")` |
| ⏹️ `stop` | 关闭通道：强度归零 + 停止波形 | `stop("A")` / `stop()` 全关 |
| 🎚️ `adjust_strength` | 运行中调强度（不换波形），边缘控制常用 | `adjust_strength("A", 3)` |
| 🔁 `change_wave` | 运行中换波形（不动强度），从波形库里选 | `change_wave("A", waveform="tide")` |
| ⚡ `burst` | 运行中短时突增强度，≤ 5 秒后自动回落（安全硬保证） | `burst("A", strength=40, duration_ms=2000)` |
